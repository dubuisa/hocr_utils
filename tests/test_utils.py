...
def test_get_page_request_existing_page():
    body = ' '.join(
        [f'<div class="ocr_page" id="page_{i+1}"></div>' for i in range(2)]
    )
...
def test_len_pages(expected_length):
    body = ' '.join(
        [f'<div class="ocr_page" id="page_{i+1}"></div>' for i in range(expected_length)]
    )
    hocr = f"<html><body>{body}</body></html>"
...
def test_get_lang():
    body = ' '.join(
        [f'<span class="ocr_line" lang="{lang}"></span>' for lang in ['fra', 'eng', 'eng']]
    )
    hocr = f"<html><body>{body}</body></html>"
...
@pytest.mark.parametrize("by,text,n_element,id_,x1,y1,x2,y2", 
                         [
...
@pytest.mark.parametrize("pdf_path,pdf_bytes,hocrs,hocr_dict,page_id", 
                         [
...
def test_draw_pdf_with_boxes(pdf_path,pdf_bytes,hocrs,hocr_dict,page_id,mocker):
    ...
    m3 = mocker.patch('hocr_utils.utils.hocr_as_dict', return_value=[{'page': 0}, {'page': 0}, {'page': 1}])
